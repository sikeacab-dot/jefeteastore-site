var products = [
{
"id": 1785282790669,
"name": "Квітковий чай",
"category": "Green",
"price": 400,
"variants": {
"50": 200,
"100": 400
},
"image": "assets/products/flowertea2.webp",
"images": [
"assets/products/flowertea2.webp"
],
"origin": "",
"description": "Квітковий чай у бутонах, викpучений вручну. При заварюванні кулька розпускається в чайнику, перетворюючись на вишукану живу квітку. Дарує ніжний свіжий смак із квітковим ароматом, перетворюючи звичайне чаювання на естетичний ритуал.",
"on_order": false,
"badge": "none",
"brewing": {
"steeps": 1,
"time": 20,
"grams": 10
}
},
{
"id": 1785281861617,
"name": "Габа \"Білій чай\"",
"category": "Gaba",
"price": 600,
"variants": {
"50": 350,
"100": 600
},
"image": "assets/products/gabawhitetea2.webp",
"images": [
"assets/products/gabawhitetea2.webp",
"assets/products/gabawhitetea.webp"
],
"origin": "",
"description": "Перший у світі габований білий чай - унікальний експеримент, що поєднує ніжність юньнанської сировини та глибокий релакс-ефект GABA. Має надзвичайно тонкий, витончений смак із вираженими пряними нотами, медовою солодкістю та м'яким квітковим післясмаком.",
"on_order": false,
"badge": "none",
"brewing": {
"steeps": 5,
"time": 20,
"grams": 8
}
},
{
"id": 1785280971599,
"name": "Габа \"Червоний Корал\"",
"category": "Gaba",
"price": 600,
"variants": {
"50": 350,
"100": 600
},
"image": "assets/products/gabaredcorall.webp",
"images": [
"assets/products/gabaredcorall.webp"
],
"origin": "",
"description": "Високогірна вулканічна GABA, вирощена на багатих мінералами ґрунтах згаслого вулкана. Має витончений, легкий та природно солодкуватий смак із м’яким карамельно-фруктовим відтінком. Чудовий вечірній чай для зняття напруги та глибокого релаксу.",
"on_order": false,
"badge": "NEW",
"brewing": {
"steeps": 7,
"time": 20,
"grams": 8
}
},
{
"id": 1782017223240,
"name": "Шу Пуер \"Стовб Дракона\"",
"category": "Puerh",
"price": 300,
"variants": {
"50": 200,
"100": 300,
"357": 1050
},
"image": "assets/products/shudragon1.webp",
"images": [
"assets/products/shudragon1.webp",
"assets/products/shudragon2.webp",
"assets/products/shudragon3.webp"
],
"origin": "",
"description": "Класика шу пуерів з Юнаню, ідеально розкривається за будь-яких умов: від класичного чаювання до заварювання в термосі.",
"on_order": false,
"badge": "NEW",
"brewing": {
"steeps": 6,
"time": 20,
"grams": 8
}
},
{
"id": 1782016895994,
"name": "Шу Пуер \"Старі Бруньки\"",
"category": "Puerh",
"price": 500,
"variants": {
"50": 300,
"100": 500
},
"image": "assets/products/shubrunki.webp",
"images": [
"assets/products/shubrunki.webp"
],
"origin": "",
"description": "Купаж від фабрики Фуцзянь із рекордною кількістю золотих бруньок, які дарують напою виняткову м'якість та глибокий, оксамитовий смак.",
"on_order": false,
"badge": "NEW",
"brewing": {
"steeps": 5,
"time": 20,
"grams": 8
}
},
{
"id": 1782016421729,
"name": "Габа \"Золото\"",
"category": "Gaba",
"price": 600,
"variants": {
"50": 350,
"100": 600
},
"image": "assets/products/gabagold.webp",
"images": [
"assets/products/gabagold.webp"
],
"origin": "",
"description": "Сорт габи, маючий характерний золотий настій з обволікаючим смаком печеного яблука, пряного меду та фірмовою фруктовою кислинкою.",
"on_order": false,
"badge": "NEW",
"brewing": {
"steeps": 8,
"time": 20,
"grams": 8
}
},
{
"id": 1780525280220,
"name": "Габа \"Те Гуань Інь\"",
"category": "Gaba",
"price": 550,
"variants": {
"50": 300,
"100": 550
},
"image": "assets/products/Gabatieguanyin.webp",
"images": [
"assets/products/Gabatieguanyin.webp"
],
"origin": "",
"description": "Габований Те Гуань Інь, з великою концентрацію ГАМК. Створений для поціновачів заспокійливого ефекту габи, та смаку класичного Те Гуань Інь.",
"on_order": false,
"badge": "none",
"brewing": {
"steeps": 7,
"time": 20,
"grams": 8
}
},
{
"id": 1780525093170,
"name": "Габа \"Дикий мед\"",
"category": "Gaba",
"price": 600,
"variants": {
"50": 350,
"100": 600
},
"image": "assets/products/Gabawildhoney.webp",
"images": [
"assets/products/Gabawildhoney.webp"
],
"origin": "",
"description": "Перша в світі габа з натуральним диким медом. Дуже смачна габа з нотами карамелі, квітів, ванилі, випічки, та медової солодкості. Сорт створений за стандартною габа технологією, без додавання ароматизаторів.",
"on_order": false,
"badge": "HIT",
"brewing": {
"steeps": 6,
"time": 20,
"grams": 7
}
},
{
"id": 1780524614186,
"name": "Габа Шу Пуер",
"category": "Gaba",
"price": 600,
"variants": {
"50": 350,
"100": 600
},
"image": "assets/products/Gabashupuerh1.webp",
"images": [
"assets/products/Gabashupuerh1.webp",
"assets/products/Gabashupuerh2.webp",
"assets/products/Gabashupuerh3.webp"
],
"origin": "Менхай, Китай",
"description": "Чайний лист, пройшовший стандартну для шу пуеру ферментацію та додатково габований. Має незвичайний, легкий, трохи солодкуватий смак. Вечірній варіант з ледь замітною землянистою нотою шу пуеру.",
"on_order": false,
"badge": "NEW",
"brewing": {
"steeps": 4,
"time": 20,
"grams": 8
}
},
{
"id": 1779931542738,
"name": "Шен Пуер \"Точа\"",
"category": "Puerh",
"price": 400,
"variants": {
"50": 250,
"100": 400
},
"image": "assets/products/shengtuo5.webp",
"images": [
"assets/products/shengtuo5.webp",
"assets/products/shengtuo1.webp",
"assets/products/shengtuo2.webp",
"assets/products/shengtuo3.webp",
"assets/products/shengtuo4.webp"
],
"origin": "Юнань, Китай",
"description": "Класичний Шен пуер, котрий дарує бодрість, концентрацію та прекрасно доповнює будь який момент. Заварювати бажано при 90-95 градусах, та проливами не більше 5 секунд.",
"on_order": false,
"badge": "none",
"brewing": {
"steeps": 0,
"time": 20,
"grams": 0
}
},
{
"id": 1776520434777,
"name": "Шу Пуер \"Точа\"",
"category": "Puerh",
"price": 500,
"variants": {
"50": 300,
"100": 500
},
"image": "assets/products/tuocha2.webp",
"images": [
"assets/products/tuocha2.webp",
"assets/products/toucha1.webp"
],
"origin": "Юнань, Китай",
"description": "Легендарний шу пуер, рецепт котрого створювався для експорту пуеру в францію, але він вийшов таким смачним, що став топовою позицію флагманської фабрики \"Xiaguan\"",
"on_order": false,
"badge": "none",
"brewing": {
"steeps": 7,
"time": 20,
"grams": 8
}
},
{
"id": 1775867642449,
"name": "Габа \"Да Хун Пао\"",
"category": "Gaba",
"price": 600,
"variants": {
"50": 350,
"100": 600
},
"image": "assets/products/GabaDPH2.webp",
"images": [
"assets/products/GabaDPH2.webp",
"assets/products/GabaDHP1.webp"
],
"origin": "Китай, провінція Фуцзянь, гори Уї",
"description": "Унікальне поєднання скельного характеру Да Хун Пао та заспокійливої дії Габа-чаю. У смаку відчуваються пряні нотки вогню, печеної корочки хліба та легка ягідна кислинка. Цей чай м’яко \"перезавантажує\" нервову систему, знімаючи напругу після складного дня, при цьому залишаючи розум ясним.",
"on_order": false,
"badge": "HIT",
"brewing": {
"steeps": 5,
"time": 20,
"grams": 8
}
},
{
"id": 1779933225171,
"name": "Шу Пуер 908",
"category": "Puerh",
"price": 550,
"variants": {
"50": 300,
"100": 550,
"250": 1300
},
"image": "assets/products/shu9081.webp",
"images": [
"assets/products/shu9081.webp",
"assets/products/shu9082.webp",
"assets/products/shu9083.webp"
],
"origin": "Юнань, Китай",
"description": "Відомий 908 рецепт приготування пуеру від фабрики Haiwan, дуже зручний варіант щоб завжди мати з собою порцію улюбленого чаю. Також ідеально підійде до знайомства з пуерами.",
"on_order": false,
"badge": "none",
"brewing": {
"steeps": 0,
"time": 20,
"grams": 0
}
},
{
"id": 1779931949990,
"name": "Шу Пуер \"7663\" Точа",
"category": "Puerh",
"price": 500,
"variants": {
"50": 300,
"100": 500
},
"image": "assets/products/shu7663tuo1.webp",
"images": [
"assets/products/shu7663tuo1.webp",
"assets/products/shu7663tuo2.webp",
"assets/products/shu7663tuo3.webp",
"assets/products/shu7663tuo4.webp",
"assets/products/shu7663tuo5.webp"
],
"origin": "Юнань, Китай",
"description": "Шу Пуер, приготовлений за багатолітнім рецептом, котрий вражає поціновачів вже понад 50 років. Має класичний пуерний смак, з земельними, древесними та глибокими нотами.",
"on_order": true,
"badge": "none",
"brewing": {
"steeps": 0,
"time": 20,
"grams": 0
}
},
{
"id": 1779932529362,
"name": "Шу Пуер Гунтін \"Зірки\" ",
"category": "Puerh",
"price": 400,
"variants": {
"50": 250,
"100": 400,
"357": 1300
},
"image": "assets/products/shustars.webp",
"images": [
"assets/products/shustars.webp",
"assets/products/shustars4.webp",
"assets/products/shustars3.webp",
"assets/products/shustars2.webp"
],
"origin": "Юнань, Китай",
"description": "Чайний лист, зібраний з старих дерев в 2013 році, котрий пройшов ферментацію та отримав дуже легкий, незвичний для пуеру смак. Для поціновачів витриманих пуерів з великою кількістю бруньок. ",
"on_order": false,
"badge": "NEW",
"brewing": {
"steeps": 7,
"time": 20,
"grams": 8
}
},
{
"id": 1779932202777,
"name": "Шу Пуер \"Кирпич\"",
"category": "Puerh",
"price": 400,
"variants": {
"50": 250,
"100": 400,
"250": 1000
},
"image": "assets/products/Shubricktuo2.webp",
"images": [
"assets/products/Shubricktuo2.webp",
"assets/products/Shubricktuo1.webp"
],
"origin": "Юнань, Китай",
"description": "Пресований в брикет Шу пуер від легендарної фабрики Xiaguan.",
"on_order": false,
"badge": "NEW",
"brewing": {
"steeps": 0,
"time": 20,
"grams": 0
}
},
{
"id": 1775867464850,
"name": "Молочний улун",
"category": "Oolong",
"price": 350,
"variants": {
"50": 200,
"100": 350
},
"image": "assets/products/Milkoolong.webp",
"images": [
"assets/products/Milkoolong.webp"
],
"origin": "Китай",
"description": "Ніжний чай з високогір’я, що поєднує свіжість зеленого листа та м’якість вершків. Смак збалансований, без терпкості, з приємною природною солодкістю. Гарний вибір, щоб познайомитися з китайською традицією та відновити сили протягом дня.",
"on_order": false,
"badge": "HIT",
"brewing": {
"steeps": 5,
"time": 20,
"grams": 7
}
},
{
"id": 1779654407869,
"name": "Шу Пуер \"Точа 23\"",
"category": "Puerh",
"price": 500,
"variants": {
"50": 300,
"100": 500,
"250": 1200
},
"image": "assets/products/toucha250.3.webp",
"images": [
"assets/products/toucha250.3.webp",
"assets/products/toucha250.5.webp",
"assets/products/toucha250.4.webp",
"assets/products/toucha250.1.webp",
"assets/products/toucha250.2.webp"
],
"origin": "Китай, провінція Юньнань",
"description": "Китайці називають його \"Фуо\" або \"Маленька Франція\". Флагманська фабрика \"Xiaguan\", 2023 рік. Не дивлячись на недовгу витримку має чистий смак, без земляної вогкості.",
"on_order": false,
"badge": "none",
"brewing": {
"steeps": 5,
"time": 20,
"grams": 8
}
},
{
"id": 1776520702809,
"name": "Фен Хуан Дан Цун",
"category": "Oolong",
"price": 550,
"variants": {
"50": 300,
"100": 550
},
"image": "assets/products/fxdc.webp",
"images": [
"assets/products/fxdc.webp"
],
"origin": "Гори \"Фен Хуан\", провінція Гуандун",
"description": "Легендарний гуандунський улун, що славиться своїм феноменальним ароматом, який порівнюють з пахощами диких орхідей та меду. Це справжній «чайний парфум», який став візитівкою гір Фенікса завдяки своєму яскравому багатогранному смаку та сильному енергійному стану.",
"on_order": false,
"badge": "SALE",
"brewing": {
"steeps": 7,
"time": 20,
"grams": 8
}
},
{
"id": 1775867761283,
"name": "Габа \"Сапфір\"",
"category": "Gaba",
"price": 550,
"variants": {
"50": 300,
"100": 550
},
"image": "assets/products/Gabasaphire.webp",
"images": [
"assets/products/Gabasaphire.webp"
],
"origin": "Китай, провінція Юньнань",
"description": "Вишуканий Габа-улун з вулканічних високогір’їв Тенчуна (провінція Юньнань). Завдяки багатим на мінерали ґрунтам, чай має глибокий, чистий смак з нотами ягідного джему та ледь помітною прохолодою м’яти. Він м'яко занурює у стан глибокого спокою, наче очищуючи думки від зайвого шуму.",
"on_order": true,
"badge": "HIT",
"brewing": {
"steeps": 8,
"time": 20,
"grams": 8
}
},
{
"id": 1775867846686,
"name": "Да Хун Пао \"Вогняний\"",
"category": "Oolong",
"price": 400,
"variants": {
"50": 250,
"100": 400
},
"image": "assets/products/DHP.webp",
"images": [
"assets/products/DHP.webp"
],
"origin": "Китай, північ провінції Фуцзянь",
"description": "Еталонний скельний улун з північної Фуцзяні, що втілює тисячолітню чайну традицію гір Уї. У кожній піалі розкривається міцний характер: ноти підсмаженого хліба, стиглих фруктів та легкий серпанок деревного вугілля. Чай, що зігріває зсередини, дарує ясність думок та непохитний внутрішній спокій.",
"on_order": false,
"badge": "HIT",
"brewing": {
"steeps": 7,
"time": 20,
"grams": 8
}
},
{
"id": 1775867941281,
"name": "Женьшень Улун",
"category": "Oolong",
"price": 350,
"variants": {
"50": 200,
"100": 350
},
"image": "assets/products/Ginseng.webp",
"images": [
"assets/products/Ginseng.webp"
],
"origin": "Китай, провінція Фуцзянь",
"description": "Традиційний улун у формі щільно пресованих гранул, насичених екстрактом женьшеню. При заварюванні «камінці» повільно розкриваються, віддаючи напою пряність та густину. Це чай з характером, який окутує теплом та залишає в роті приємну солодкість, що тримається годинами.",
"on_order": false,
"badge": "none",
"brewing": {
"steeps": 6,
"time": 20,
"grams": 8
}
},
{
"id": 1775868178832,
"name": "Те Гуань Інь \"Лао Чжу\"",
"category": "Oolong",
"price": 350,
"variants": {
"50": 200,
"100": 350
},
"image": "assets/products/Tieguanyin.webp",
"images": [
"assets/products/Tieguanyin.webp"
],
"origin": "Китай, південь провінції Фуцзянь",
"description": "Світлий улун з південної Фуцзяні в його найбільш природному, «фермерському» вигляді. Наявність чайних черешків дарує настою особливу медову солодкість та густину, якої немає в рафінованих сортах. Смак свіжої весняної зелені з ароматом бузку та довгим, соковитим післясмаком.",
"on_order": false,
"badge": "none",
"brewing": {
"steeps": 6,
"time": 20,
"grams": 7
}
},
{
"id": 1776520969142,
"name": "Уі Шуй Сянь \"Нарцисс\" (Порційний)",
"category": "Oolong",
"price": 600,
"variants": {
"50": 350,
"100": 600
},
"image": "assets/products/narciss2.2.webp",
"images": [
"assets/products/narciss2.2.webp",
"assets/products/narciss2.1.webp"
],
"origin": "Гори Уї (Уїшань)",
"description": "Цей сорт став еталоном скельних улунів завдяки глибокому, багатогранному смаку з м’якими деревними та медовими нотами, а також сильному заспокійливому ефекту.",
"on_order": false,
"badge": "none",
"brewing": {
"steeps": 7,
"time": 20,
"grams": 7
}
},
{
"id": 1,
"name": "Габа \"King Gaba\"",
"category": "Gaba",
"price": 750,
"variants": {
"50": 400,
"100": 750
},
"image": "assets/products/prod_1_0.webp",
"images": [
"assets/products/prod_1_0.webp"
],
"origin": "Тайвань",
"description": "Преміальний улун, ферментований для збагачення ГАМК. Солодкуватий фруктово-медовий смак із карамельною нотою.\n\nЧай для розслаблення, спокою та ясності мислення.",
"on_order": false,
"badge": "HIT",
"brewing": {
"steeps": 7,
"time": 20,
"grams": 7
}
},
{
"id": 1775868050998,
"name": "Уі Шуй Сянь \"Нарцисс\"",
"category": "Oolong",
"price": 375,
"variants": {
"50": 225,
"100": 375
},
"image": "assets/products/Narciss.webp",
"images": [
"assets/products/Narciss.webp"
],
"origin": "Китай, північ провінції Фуцзянь",
"description": "Традиційний темний улун з північної Фуцзяні, що росте на багатих мінералами скелях. Має насичений, густий настій з відтінками вологої деревини, меду та весняних квітів. Нарцис вирізняється особливою м’якістю та «об'ємом» у роті, що робить кожне ковтання справжньою церемонією.",
"on_order": false,
"badge": "none",
"brewing": {
"steeps": 7,
"time": 20,
"grams": 8
}
},
{
"id": 1771117657775,
"name": "Смола Пуеру \"Ча Гао\"",
"category": "Puerh",
"price": 200,
"variants": null,
"image": "assets/products/image_4.webp",
"images": [
"assets/products/image_4.webp"
],
"origin": "Юньнань, Китай",
"description": "Концентрований екстракт пуеру з дуже насиченим, «ядерним» смаком. Глибокі ноти темного пуеру, щільність та інтенсивність у кожному ковтку.\n\nЧай для тонусу, концентрації та швидкого ефекту.\n\nЦіна вказана за 20 шт.",
"on_order": false,
"badge": "none",
"brewing": {
"steeps": 2,
"time": 20,
"grams": 1
}
},
{
"id": 3,
"name": "Да Хун Пао (пачки)",
"category": "Oolong",
"price": 250,
"variants": {
"100": 250,
"200": 450
},
"image": "assets/products/prod_3_0.webp",
"images": [
"assets/products/prod_3_0.webp",
"assets/products/prod_3_1.webp"
],
"origin": "Уїшань, Китай",
"description": "Класичний високоякісний Да Хун Пао у порційних пачках. Збалансований, чистий смак із нотами карамелі, легкої деревини та кам’янистої мінеральності. Аромат глибокий, післясмак довгий та теплий.\n\nЧай для тонусу, концентрації та яскравих смакових відчуттів.",
"on_order": true,
"badge": "none",
"brewing": {
"steeps": 7,
"time": 20,
"grams": 7
}
},
{
"id": 6,
"name": "Да Хун Пао",
"category": "Oolong",
"price": 350,
"variants": {
"50": 200,
"100": 350
},
"image": "assets/products/prod_6_0.webp",
"images": [
"assets/products/prod_6_0.webp"
],
"origin": "Уїшань, Китай",
"description": "Легендарний улун із скельним характером та глибоким ароматом. Має дуже яскравий, насичений смак із нотами пряності, з відтінками деревини, смажених горіхів і печеного каштана, та довгим солодкуватим післясмаком.\n\nЧай для тонусу, концентрації та яскравих смакових відчуттів.",
"on_order": false,
"badge": "SALE",
"brewing": {
"steeps": 6,
"time": 20,
"grams": 8
}
},
{
"id": 1775868453792,
"name": "Шу Пуер «Сім Синів» (Гун Тін)",
"category": "Puerh",
"price": 0,
"variants": null,
"image": "assets/products/yunanpuer1.webp",
"images": [
"assets/products/yunanpuer1.webp",
"assets/products/yunanpuer2.webp"
],
"origin": "Китай, провінція Юньнань",
"description": "Еталонний Шу Пуер з Меньхаю, виготовлений з добірної «палацової» сировини з великою кількістю золотистих бруньок. Настій густий та темний, з виразним ароматом лісового горіха, чорного шоколаду та благородної деревини. Чай, що миттєво збирає думки до купи, дарує потужний заряд енергії та зігріває з першого ковтка.",
"on_order": true,
"badge": "NEW",
"brewing": {
"steeps": 6,
"time": 20,
"grams": 8
}
},
{
"id": 5,
"name": "Пуер \"Менхай\"",
"category": "Puerh",
"price": 275,
"variants": {
"50": 150,
"100": 275,
"357": 500
},
"image": "assets/products/prod_5_0.webp",
"images": [
"assets/products/prod_5_0.webp",
"assets/products/prod_5_1.webp",
"assets/products/prod_5_2.webp"
],
"origin": "Юньнань, Китай",
"description": "Класичний витриманий шу пуер у формі млинця. Не надто гіркий, із землянистими нотами, деревними відтінками та легкою солодкістю.\n\nЧай для тепла, спокою та м’якої концентрації.",
"on_order": true,
"badge": "SALE",
"brewing": {
"steeps": 6,
"time": 20,
"grams": 8
}
}
];